...
- 远程管理端在修改 `remoteServiceToken` 或同端口重启配置后，会重新启动服务，避免继续沿用旧令牌。
- 远程 client 模式新增分集下载链接刷新接口：
  - `POST /api/short-dramas/:code/episodes/:episodeIndex/refresh`
  - 当分集 `download_url` 过期时，客户端会先请求管理端刷新，再重新拉取最新分集列表并继续加入下载队列。
- 自动“请求准备资源”流程改为仅在请求成功后记录已请求状态；如果首次请求失败，后续仍可自动再次尝试，不会卡死在失败后的静默状态。

### 远程模式排查建议

- 如果朋友端能看到剧集但点击下载无反应，先确认管理端已经保存最新夸克 Cookie，并在管理端重新进入该剧一次，让分集链接完成刷新。
- 如果刚修改过远程访问令牌，请让客户端改用新令牌重新连接；管理端现在会立即按新令牌校验。
- 如果远程客户端之前因为网络波动导致“自动准备资源”失败，重新选中该剧后会再次自动发起请求，也可以手动点击“请求准备资源”重试。
### 同机联调说明（客户端 + 管理端）

- 客户端开发模式固定使用 `http://localhost:5173`
- 管理端开发模式固定使用 `http://localhost:5174`
- 这样同一台机器可以同时启动两套桌面进程，不会再因为 Vite 端口冲突互相抢占
- 如果客户端要连本机管理端，管理端地址可填写 `http://localhost:39095`
